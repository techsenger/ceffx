...
/**
 * Provides the CEF version and platform information bundled into this classifier's jar, along with
 * the resulting download links for the official CEF distribution archives.
 *
 * <p>Values are read once, at class initialization time, from the {@code native.properties} resource
 * shipped alongside this class in the {@code com.techsenger.ceffx.natives.props} package. This makes the
 * required CEF version discoverable by a build tool, installer or launcher without loading the native
 * runtime - see {@link #CEF_VERSION}.
 *
...
public final class NativeProps {
     ...
     */
    public static final String CEF_LINK_STD;

    /**
     * Direct download link for the minimal CEF distribution archive matching {@link #CEF_DISTRIBUTION}.
     */
    public static final String CEF_LINK_MIN;

    private static final String CEF_BUILDS_BASE_URL = "https://cef-builds.spotifycdn.com/";
    ...
    static {
        ...
        CEF_LINK_STD = CEF_BUILDS_BASE_URL + encodedDistribution + ".tar.bz2";
        CEF_LINK_MIN = CEF_BUILDS_BASE_URL + encodedDistribution + "_minimal.tar.bz2";
    }
